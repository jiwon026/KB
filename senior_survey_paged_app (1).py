...
st.title("💬 시니어 금융 유형 진단")

# 상태 초기화
if "stage" not in st.session_state:
    st.session_state.stage = "intro"  # intro, survey, result
...
if st.session_state.stage == "intro":
    st.markdown("## 🧓 연금을 현재 수령하고 계신가요?")
    pension_status = st.radio("연금 수령 여부", ["예", "아니오"], index=None)

    if pension_status == "예":
        if st.button("설문 시작하기"):
            st.session_state.stage = "survey"
    elif pension_status == "아니오":
        st.warning("❗ 연금을 수령하고 있지 않으시군요. 연금 계산 기능은 추후에 추가될 예정입니다.")
