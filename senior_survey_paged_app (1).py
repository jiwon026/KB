...
if "pension_check" not in st.session_state:
    st.session_state.pension_check = None
...
if not st.session_state.survey_started:
    st.markdown("### 🔍 현재 연금을 수령하고 계신가요?")
    pension_status = st.radio(
        "연금 수령 여부를 선택해주세요.",
        ("예", "아니오"),
        key="pension_check"
    )
    ...
    if pension_status == "예":
        ...
        st.info("📌 연금을 수령하지 않는 경우, 추후 연금 수령액 계산 페이지로 이동합니다. (아직 미구현)")
        # 이후에 연금 계산 기능 구현 예정
# ➤ 연금 수령 중인 사람만 설문 진행
